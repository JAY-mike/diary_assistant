...
DATABASE_URL = "mysql+pymysql://root:123456@127.0.0.1:3306/diary_db"
...
#依赖注入函数，每一次请求都会创建一个新的数据库会话，并在请求结束后关闭它
    ...
    #         yield session
def get_session():
    session = SessionLocal()  #创建一个新的数据库会话
    try:
        yield session  #将会话对象提供给请求处理函数使用
    finally:
        session.close()  #确保请求结束后关闭会话，释放数据库连接
...
def create_diary(diary: DiaryCreate,session: Session = Depends(get_session)):
    """
    写一篇新日记
    # """
    ...
    session.refresh(diary)  #刷新对象以获取数据库生成的ID和时间等信息
    return diary
...
def read_diaries(skip: int = 0, limit: int = 10, session: Session = Depends(get_session)):
    """
    获取日记列表(支持offset/limit分页)
    """
    statement = select(Diary).offset(skip).limit(limit)  #构建查询语句，支持分页
    diaries = session.exec(statement).all()  #执行查询并获取结果
    return diaries

@app.get("/diaries/{diary_id}",response_model=DiaryRead)
def read_diary(diary_id: int, session: Session = Depends(get_session)):
    """
    获取指定ID的日记
    """
    diary = session.get(Diary, diary_id)  #根据ID获取日记
    if not diary:
        raise HTTPException(status_code=404, detail="Diary not found")  #如果没有找到，抛出404错误
    return diary

@app.delete("/diaries/{diary_id}")
def delete_dairy(diary_id: int, session: Session = Depends(get_session)):
    """
    删除指定ID的日记
    """
    diary = session.get(Diary, diary_id)  #根据ID获取日记
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="127.0.0.1", port=8001, reload=True)  #启动FastAPI应用，reload=True表示代码修改后自动重启服务器